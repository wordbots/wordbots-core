...
describe('Game reducer', () => {
  ...
  describe('[Activated abilities]', () => {
    it('should let objects activate activated abilities when able', () => {
      ...
      let state = setUpBoardState({
        'blue': {
          '2,0,-2': cards.fortificationCard
        }
      });
      let currentHand = hand();

      // Recycler: "Activate: Discard a card, then draw a card."
      state = playObject(state, 'orange', cards.recyclerCard, '3,0,-3');

      // Can't activate the turn it's played.
      state = activate(state, '3,0,-3', 0, {card: 0});
      ...
      state = attack(state, '3,0,-3', '2,0,-2');
      expect(queryObjectAttribute(state, '2,0,-2', 'health')).toEqual(cards.fortificationCard.stats.health);

      // Can't attack then activate on the same turn.
      state = newTurn(state, 'orange');
      state = attack(state, '3,0,-3', '2,0,-2');
      expect(queryObjectAttribute(state, '2,0,-2', 'health')).toEqual(cards.fortificationCard.stats.health - 1);
      currentHand = hand();
      state = activate(state, '3,0,-3', 0, {card: 0});
      expect(hand()).toEqual(currentHand);
    });
