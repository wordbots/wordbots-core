      ...
      if (this.props.output === 'html') {
        return (
          <img
            className="wb-sprite"
            src={dataURL}
            width={size * (this.props.scale || 1)}
            height={size * (this.props.scale || 1)}
  ...
  drawSprite(elt, props) {
    ...
    ByteMap.prototype = Object.create(Uint8Array.prototype, {
      set: function (x, y, value){
        this[indexFromCoord(this.width, x, y)] = value;
      },
      get: function (x, y){
        return this[indexFromCoord(this.width, x, y)];
      }
    });
    ...
    LinearCongruentialGenerator.prototype = {
      ...
      reseed: function (seed){
        this.seed = (seed ? seed : Date.now()) % this.m;
      }
    };
    ...
    function scaleEagle2x(data, x, y){
      ...
      const I = getPixel(
        data,
        x < data.width - 1 ? x + 1 : data.width - 1,
        y < data.height - 1 ? y + 1 : data.height - 1
      );
      return [
    ...
    const palettes = {
      ...
      gameboy: {
        label: 'Gameboy',
        background: 0,
        colours: [ [ 57, 57, 41 ], [ 123, 115, 99 ], [ 181, 165, 107 ], [ 231, 214, 156 ] ]
      },
      greys: {
        label: 'Greys',
        background: 0,
      ...
      for (let y = 0; y < yLimit; y++) {
        for (let x = 0; x < xLimit; x++) {
          ...
          const prob = lerp(
            ...
            bias(this.options['bias'], gain(this.options['gain'], falloffX * falloffY))
          );
          const rand = random();
          if (rand <= prob) putPixel(tile, x, y, rgbToRgba(pickRandom(spritePal)));
          else putPixel(tile, x, y, [ 255, 255, 255, 0 ]);
        }
      }
      // do post
      for (let y = 0; y < yLimit; y++) {
    ...
    SpriteGenerator.prototype.getScale = function (){
      return (
        scalers[this.options['scaler0']].factor *
        scalers[this.options['scaler1']].factor /** this.options["iterations"]*/
      );
    };
    ...
    const properties = {
      ...
      pal: {
        ...
        default: 'arne',
        options: palettes,
        description: 'Palette to draw colours from.'
      },
      colours: {
        type: 'integer',
        label: 'Colours',
        ...
        min: 1,
        max: 16,
        description: 'Number of colours per sprite.'
      },
      ...
      scaler0: {
        ...
        default: 'eagle2x',
        options: scalers,
        description: 'First sprite scaling method.'
      },
      scaler1: {
        type: 'list',
        label: 'Scaler 1',
        default: 'none',
        options: scalers,
        description: 'Second sprite scaling method.'
      },
      falloff: {
        type: 'list',
        label: 'Falloff',
      ...
      probmin: {
        ...
        min: 0,
        max: 1,
        description: 'Probability of pixel placement at the edge of the sprite.'
      },
      probmax: {
        type: 'real',
        label: 'Max',
        ...
        min: 0,
        max: 1,
        description: 'Probability of pixel placement at the centre of the sprite.'
      },
      bias: {
        type: 'real',
        label: 'Bias',
      ...
      mirrorh: {
        ...
        min: 0,
        max: 1,
        description: 'Probability of the sprite being horizontally mirrored.'
      },
      mirrorv: {
        type: 'real',
        label: 'Vertical',
        ...
        min: 0,
        max: 1,
        description: 'Probability of the sprite being vertically mirrored.'
      },
      despeckle: {
        type: 'real',
        label: 'Despeckle',
        ...
        min: 0,
        max: 1,
        description: 'Probability of a single pixel being despeckled.'
      },
      despur: {
        type: 'real',
        label: 'Despur',
        ...
        min: 0,
        max: 1,
        description: 'Probability of a single jutting pixel being removed.'
      }
    };
