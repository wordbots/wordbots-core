...
import { compact, pull, reject } from 'lodash';
...
function withoutClient(game, clientID) {
  const updatedGame = Object.assign({}, game, {
    ...
    spectators: game.players.filter(p => p !== clientID)
  });
...
export default class MultiplayerServerState {
  ...
  getClientSocket(clientID) {
    return this.state.connections[clientID];
  }
  ...
  getClientSockets(clientIDs = null) {
    return clientIDs ? clientIDs.map(this.getClientSocket) : Object.values(this.state.connections);
  }

  // Returns the username to use for the given player.
  getClientUsername(clientID) {
    return this.state.usernames[clientID] || clientID;
  }

  // Returns the game that the given player is in, if any.
  lookupGameByClient(clientID) {
    return this.state.games.find(g => g.players.includes(clientID));
  }

  // Returns all *other* players in the game that the given player is in, if any.
  getAllOpponents(clientID) {
    const game = this.state.games.find(g => g.players.includes(clientID) || g.spectators.includes(clientID));
    if (game) {
  ...
  disconnectClient(clientID) {
    ...
    const game = this.state.lookupGameByClient(clientID);
    if (game) {
  ...
  // Add an action to a player's game.
  ...
  // Make a player host a game with the given game name and using the given deck.
  ...
  joinGame(clientID, opponentID, deck) {
    const opponent = this.state.waitingPlayers.find(p => p.id === opponentID);
    const game = {
      id: opponentID,
      name: opponent.name,
