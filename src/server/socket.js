...
export default function launchWebsocketServer (server, path){
  ...
  function findOpponents (clientID){
    ...
    if (game) {
      return game.players.concat(game.spectators).filter(id => id !== clientID);
    }
  }
  ...
  function joinGame (clientID, opponentID, deck){
    ...
    sendMessage('ws:GAME_START', {player: 'blue', decks: decks, usernames: usernames, seed: seed}, [
      clientID
    ]);
    sendMessage(
      'ws:GAME_START',
      {player: 'orange', decks: decks, usernames: usernames, seed: seed},
      [ opponentID ]
    );
